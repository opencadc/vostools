...
************************************************************************
*/
package ca.nrc.cadc.adql.converter;

import org.apache.log4j.Logger;

import ca.nrc.cadc.adql.AdqlManager;
import ca.nrc.cadc.adql.config.AdqlConfig;
import ca.nrc.cadc.adql.impl.postgresql.pgsphere.converter.ConverterImpl;
...
public abstract class Converter implements SelectVisitor {
//	public class Converter implements SelectVisitor, SelectItemVisitor, FromItemVisitor, OrderByVisitor {
    protected static Logger log = Logger.getLogger(Converter.class);

    protected AdqlManager manager;
    protected AdqlConfig config; 
    protected TopConverter topConverter;
    protected RegionConverter regionConverter;
    protected AllColumnsConverter allColumnsConverter;

    
	public void init(AdqlManager manager) {
    	this.manager = manager;
    	this.config = manager.getConfig();
    	topConverter.init(manager);
    	regionConverter.init(manager);
    	allColumnsConverter.init(manager);
    }
	@Override
    public void visit(PlainSelect ps)
    {
        ...
        topConverter.visit(ps);
        regionConverter.visit(ps);
        allColumnsConverter.visit(ps);
    }
