...
public class ColumnReferenceValidatorImpl extends ColumnReferenceValidator
{
    ...
    {
        log.debug("visit(Column): " + c);
        if (this.config.isColumnAmbiguous(c, this.plainSelect))
        	addException(new AdqlValidateException(c.toString() + " is ambigious."));
        if (!this.config.isColumnValid(c, this.plainSelect))
        	addException(new AdqlValidateException(c.toString() + " is not in the from table or join tables."));
    }

    /**
     * Visit a column reference in a GROUP BY or ORDER BY. The implementation simply
     * logs the visit at debug mode.
     * 
     * @param i
     */
