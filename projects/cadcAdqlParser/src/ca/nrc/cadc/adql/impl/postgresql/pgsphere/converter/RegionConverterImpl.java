...
package ca.nrc.cadc.adql.impl.postgresql.pgsphere.converter;

import ca.nrc.cadc.adql.converter.RegionConverter;
import ca.nrc.cadc.adql.impl.postgresql.pgsphere.Constants;
import ca.nrc.cadc.observation.GeomUtil;
...
public class RegionConverterImpl extends RegionConverter
{
	public void visit(SubSelect subSelect) {
		//TODO:sz: check and convert functions in SubSelect.
	}

	public void visit(SubJoin subjoin) {
		//TODO:sz: check and convert functions in Subjoin
	}
    ...
    public void visit(PlainSelect ps)
    {
        ...
        if (ps.getSelectItems() != null)
        {
            ...
            while (i.hasNext())
            {
                ...
                if (obj instanceof SelectExpressionItem)
                {
                    ...
                    SelectExpressionItem replacement = checkSelectExpression(s);
                    if (replacement != null)
                    {
                        log.debug("PlainSelect/SELECT: replacing " + s + " with " + replacement);
                        i.set(replacement);
                    }
                }
            }
        ...
        if (ps.getJoins() != null)
        {
            ...
            while (i.hasNext())
            {
                ...
                if (j.getOnExpression() != null)
                {
                    ...
                    Expression replacement = checkExpression(e);
                    if (replacement != null)
                    {
                        log.debug("PlainSelect/JOIN...ON: replacing " + e + " with " + replacement);
                        j.setOnExpression(replacement);
                    }
                }
                //sz
        ...
        if (ps.getWhere() != null)
        {
            ...
            if (replacement != null)
            {
                log.debug("PlainSelect/WHERE: replacing " + e + " with " + replacement);
                ps.setWhere(replacement);
            }
        ...
        if (ps.getHaving() != null)
        {
            ...
            if (replacement != null)
            {
                log.debug("PlainSelect/HAVING: replacing " + e + " with " + replacement);
            }
        }
    ...
    // replace a region function in the SELECT list with something else
    private SelectExpressionItem checkSelectExpression(SelectExpressionItem e)
    {
        log.debug("checkSelectExpressionItem: " + e);
        Expression ex = e.getExpression();
        if (ex instanceof Function)
        {
            Expression rep = checkSelectedFunction((Function) ex);
            if (rep != null)
            {
                SelectExpressionItem ret = new SelectExpressionItem();
                ret.setAlias(e.getAlias());
                ret.setExpression(rep);
                return ret;
            }
        }
        // TODO: other expressions with a region function inside?
        return null;
    }
    ...
    private Expression checkFunction(Function func)
    {
        if (Constants.REGION_FUNCTIONS.contains(func.getName().toUpperCase()))
            return checkRegionFunction(func);
        return null;
    ...
    private Expression checkRegionFunction(Function func)
    {
        String fname = func.getName().toUpperCase();

        if (Constants.COORDSYS.equals(fname))
        {
            return doScalarRegionFunction(func, null, "ICRS");
        }
        if (Constants.AREA.equals(fname))
        {
            return doScalarRegionFunction(func, "area", null);
        }
        if (Constants.CVAL1.equals(fname))
        {
            return doScalarRegionFunction(func, "cval1", null);
        }
        if (Constants.CVAL2.equals(fname))
        {
            return doScalarRegionFunction(func, "cval2", null);
        }
        log.warn("checkRegionFunction did not handle " + func);
        return null; // actually a bug to get here
    }
    ...
    private Expression[] columnToColumns(Column c)
    {
        if ( isPointType(c) )
            ...
            {
                new Column(c.getTable(), c.getColumnName()+"_x"),
                new Column(c.getTable(), c.getColumnName()+"_y"),
                new Column(c.getTable(), c.getColumnName()+"_z")
            };
        ...
        if ( isShapeType(c) )
            return new Expression[] 
            {
                ...
                new Column(c.getTable(), "x1"),
                new Column(c.getTable(), "x2"),
                new Column(c.getTable(), "y1"),
                new Column(c.getTable(), "y2"),
                new Column(c.getTable(), "z1"),
                new Column(c.getTable(), "z2")
            };
    ...
    private Expression[] centroidToColumns(Column c)
    {
        ...
        if ( isShapeType(c) )
            return new Expression[] 
            {
                new Column(c.getTable(), c.getColumnName()+"_cx"),
                new Column(c.getTable(), c.getColumnName()+"_cx"),
                new Column(c.getTable(), c.getColumnName()+"_cz"),
            };
    ...
    private Expression[] functionToColumns(Function f)
    {
        ...
        if (Constants.REGION_GEOM_FUNCTIONS.contains(fname))
        {
            ...
            log.debug("fromConstants: " + fromConstants 
                    + ", fromColumns: " + fromColumns 
                    + ", fromFunctions: " + fromFunctions);
            ...
            if (Constants.POINT.equals(fname) || Constants.CIRCLE.equals(fname))
            {
                ...
                if (fromConstants)
                {
                    try
                    {
                        double[] longlat = new double[] { toDouble(x), toDouble(y) };
                        double[] icrs = toICRS(csys, longlat);
                        double[] xyz = GeomUtil.toUnitSphere(icrs[0], icrs[1]);

                        
                        
                        if (Constants.CIRCLE.equals(fname))
                        {
                            Expression r = (Expression) expr.get(3);
                            double rad = Math.toRadians(toDouble(r)); // radius ~ arc length on unit sphere

                            ret = new Expression[6];
                            for (int i=0; i<ret.length; i++)
                                ret[i] = new DoubleValue("0.0");
                            ((DoubleValue)ret[0]).setValue(xyz[0] - rad);
                            ((DoubleValue)ret[1]).setValue(xyz[0] + rad);
                            ((DoubleValue)ret[2]).setValue(xyz[1] - rad);
                            ((DoubleValue)ret[3]).setValue(xyz[1] + rad);
                            ((DoubleValue)ret[4]).setValue(xyz[2] - rad);
                            ((DoubleValue)ret[5]).setValue(xyz[2] + rad);
                        }
                        else
                        {
                            // POINT
                            ret = new Expression[3];
                            for (int i=0; i<ret.length; i++)
                                ret[i] = new DoubleValue("0.0");
                            ((DoubleValue)ret[0]).setValue(xyz[0]);
                            ((DoubleValue)ret[1]).setValue(xyz[1]);
                            ((DoubleValue)ret[2]).setValue(xyz[2]);
                        }
                    }
                    catch(ClassCastException cex)
                    {
                        throw new UnsupportedOperationException("mixing columns, constants, and functions in geometry constructor: " + f);
                    }
                }
                ...
                if (fromFunctions) // needed when coordinates are extracted from a point | centroid
                {
                    try
                    {
                        ...
                        if ( Constants.CVAL1.equals(fx.getName().toUpperCase()) 
                                && Constants.CVAL2.equals(fy.getName().toUpperCase()) )
                        {
