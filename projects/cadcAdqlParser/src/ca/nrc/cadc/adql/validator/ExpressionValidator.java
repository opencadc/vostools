...
import ca.nrc.cadc.adql.validator.SelectValidator.PlainSelectType;

/**
 * 
 * @author pdowler
 */
public abstract class ExpressionValidator extends AdqlValidatorVisitor implements ExpressionVisitor, ItemsListVisitor {
	...
	public void validateColumn(Column c) throws AdqlValidateException {
		...
		if (count == 0) {
			...
			if (this.selectValidator.isRoot())
				throw new AdqlValidateException("Column:" + c + " is not valid.");
			else {
				if (this.selectValidator.getPlainSelectType().equals(PlainSelectType.FROM_SUB_SELECT))
					throw new AdqlValidateException("Column:" + c + " is not valid.");
				else
					this.selectValidator.getParentSelectValidator().getExpressionValidator().validateColumn(c);
			}
		} else if (count == 1) {
			return;
		} else { // count >= 2
