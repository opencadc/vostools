...
public class HttpUtil 
{
    ...
    // check accept header and pick HTML (true) or XML (default, false)
    public static boolean acceptsHTML(HttpServletRequest request)
    {
        String s =  request.getHeader("Accept");
        log.debug("checkAcceptHeader: " + s);
        if (s == null || s.trim().length() == 0)
            return false;
        List types = Arrays.asList(s.split(","));
        // TODO: sort the list by specificity
        // see http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html for details
        for (int i=0; i<types.size(); i++)
        {
            s = (String) types.get(i);
            log.debug("checkAcceptHeader: \t" + s);
