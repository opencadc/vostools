...
public class GmsClientTest extends GMSTest<GmsClient>
{
    ...
    {
        final StringBuilder XML_INPUT = new StringBuilder(128);
        XML_INPUT.append(
        ...
        setTestSubject(new GmsClient(
                new URL("http://localhost/myservice/members"))
        {
            /**
    ...
    {
        final String groupID = Long.toString(88l);
        final String memberID = Long.toString(88l);
        final User member = getTestSubject().getMember(groupID, memberID);

        assertNotNull("Member with ID 88 is a Member of Group 88.", member);
        assertEquals("Member's ID is 88.", Long.toString(88l),
                     member.getUserID());
        assertEquals("Member's Username is TESTUSERNAME.", "TESTUSERNAME",
                     member.getUsername());
    }
}
