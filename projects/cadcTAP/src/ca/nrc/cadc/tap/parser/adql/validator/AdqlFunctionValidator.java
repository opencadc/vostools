 ...
 */
public class AdqlFunctionValidator extends ExpressionValidator
{
    /**
    ...
    /**
     * POINT('ICRS GEOCENTER', 20.0, -19.5)
     * 
     * @param function
     */
    public void validatePoint(Function function)
    {
        int sizeParameters = 3;
        List<Expression> parameters = function.getParameters().getExpressions();
        if (parameters.size() != sizeParameters)
            addException(new AdqlValidateException("Number of parameters incorrect: " + function));
        else if (!(parameters.get(0) instanceof StringValue))
            addException(new AdqlValidateException("First parameter should be a string: " + function));
        else
            validateCoord(parameters.get(0), parameters.get(1), parameters.get(2));
    }

    public void validatePredicate(Function function)
    {
        int sizeParameters = 2;
        List<Expression> parameters = function.getParameters().getExpressions();
        if (parameters.size() != sizeParameters)
            addException(new AdqlValidateException("Number of parameters incorrect: " + function));
        else
    ...
    public void validateSingleArgument(Function function)
    {
        int sizeParameters = 1;
        List<Expression> parameters = function.getParameters().getExpressions();
        if (parameters.size() != sizeParameters)
            addException(new AdqlValidateException("Number of parameters incorrect: " + function));
        else
    ...
    public void validateCircle(Function function)
    {
        int sizeParameters = 4;
        List<Expression> parameters = function.getParameters().getExpressions();
        if (parameters.size() != sizeParameters)
            addException(new AdqlValidateException("Number of parameters incorrect: " + function));
        else if (!(parameters.get(0) instanceof StringValue))
    ...
    public void validatePolygon(Function function)
    {
        List<Expression> parameters = function.getParameters().getExpressions();
        int pSize = parameters.size();
        int pairs = (int) (pSize / 2);
        
        log.debug("validatePolygon() ");
        if (pairs < 2 || (pairs * 2 + 1 != pSize))
            addException(new AdqlValidateException("Number of parameters incorrect: " + function));
        else if (!(parameters.get(0) instanceof StringValue))
            addException(new AdqlValidateException("First parameter should be a string: " + function));
        else
        {
            ...
            {
                validateCoord(parameters.get(0), parameters.get(i*2+1), parameters.get(i*2+2));                
            }
        }
    }

    public void validateCoord(Expression coordsys, Expression longitude, Expression latitude)
    {
        String strCoordsys = ((StringValue) coordsys).getValue();
        if (Constants.ICRS.equalsIgnoreCase(strCoordsys))
        {
            if (longitude instanceof LongValue)
            {
                long v = ((LongValue) longitude).getValue();
                if (v < 0 || v > 360)
                    addException(new AdqlValidateException("Longitude value must between 0 and 360 degree:" + v));
            }
            if (latitude instanceof LongValue)
            {
                long v = ((LongValue) latitude).getValue();
                if (v < -90 || v > 90)
                    addException(new AdqlValidateException("Latitude value must between -90 and 90 degree:" + v));
            }
            if (longitude instanceof DoubleValue)
            {
                double v = ((DoubleValue) longitude).getValue();
                if (v < 0.0 || v > 360.0)
                    addException(new AdqlValidateException("Longitude value must between 0 and 360 degree:" + v));
            }
            if (latitude instanceof DoubleValue)
            {
                double v = ((DoubleValue) latitude).getValue();
                if (v < -90.0 || v > 90.0)
                    addException(new AdqlValidateException("Latitude value must between -90 and 90 degree:" + v));
            }
        }
